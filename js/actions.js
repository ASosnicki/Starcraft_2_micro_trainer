...
// A function that checks build queue object requirements.
function check_build_queue_object_requirements(build_queue_object) {
	var requirements_met = false;
	for (var i = 0; i > build_queue_object.requirements.length; i++) {
		switch (build_queue_object.requirements[i].condition) {
		case "SCV Count":
			if (game_object_count("SCV") >= build_queue_object.requirements.quantity) {
...
function add_action_to_the_queue() {
	var current_build_order_item = build_order.queue[0];
	action_queue.push(new action(current_build_order_item));
